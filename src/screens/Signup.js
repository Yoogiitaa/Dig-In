...
export default function Signup() {
  ...
  const handleSubmit = async (e) => {
    ...
    const response = await fetch("http://localhost:5000/api/Createuser", {
      ...
      body: JSON.stringify({ name: credentials.name, email: credentials.email,location: credentials.address , password: credentials.password})
      ...
      <Navbar />
        ...
        <div className='container' >
          <form className='w-50 m-auto mt-5 border bg-dark border-success rounded' onSubmit={handleSubmit}>
            ...
            <div className="m-3">
              <button type="button" onClick={handleClick} name="address" className=" btn btn-success">Click for current Location </button>
            </div>
            <div className="m-3">
